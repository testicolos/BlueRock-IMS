...
export async function POST(request: NextRequest) {
  try {
    ...
    if (data.sessionId) {
      ...
      `)[0];
      if (item && item.validated_at == null) {
        const target = await sql`select 1 from ims_office_validation_targets where session_id=${data.sessionId} and inventory_item_id=${item.id} limit 1`;
        if (!target.length) return fail('This item is not part of the active Office Inventory validation request', 409);
      }
    } else {
      item = (await sql`
        select i.id,i.barcode,i.name,i.category,i.manufacturer,i.model,i.serial_number,i.owner_name,
          i.condition,i.status,l.name as location_name,null::timestamptz as validated_at
        from ims_office_inventory_items i
        left join ims_locations l on l.id=i.current_location_id
        where i.barcode=${data.barcode.toUpperCase()} and i.archived=false limit 1
      `)[0];
    ...
    return ok({
      ...
      status:item.status,alreadyValidated:Boolean(data.sessionId && item.validated_at),requestActive:Boolean(data.sessionId),
    });
  } catch (error) {
