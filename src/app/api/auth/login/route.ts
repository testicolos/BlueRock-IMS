...
export async function POST(request: Request) {
  try {
    ...
    const sql = db();
    const rows = await sql`select id, username, password_hash, full_name, role from ims_users where lower(username)=lower(${parsed.data.username}) and active=true limit 1`;
    const user = rows[0];
