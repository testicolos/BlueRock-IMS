...
export const GET = withRequestTrace('app.load', async (request: NextRequest) => {
  try {
    ...
    `);
    const scanSessions = await traceStage('sessions.read', () => activeScanSessions(sql));
    if (actor.role !== 'ADMIN') return ok({ locations, materials: [], users: [], scans: [], issues: [], scanSessions });
    const materials = await traceStage('materials.read', () => sql`
    ...
    `);
    // The selected assignment column belongs to this migration, not inventory's.
    await traceStage('schema.scanner', () => ensureScannerLocationSchema());
    const users = await traceStage('users.read', () => sql`
    ...
    `);
    const scans = await traceStage('history.read', () => scanRecords(sql));
    const issues = await traceStage('issues.read', () => sql`
