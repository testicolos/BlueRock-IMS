...
export async function GET(request: NextRequest) {
  try {
    ...
    let filename;
    if (month !== null) {
      workbook = buildMonthlyScanWorkbook(await scanMonthlyReport(month));
      filename = `BlueRock-Scan-Monthly-${month}.xlsx`;
    } else {
      const report = await scanChecklist(period);
      workbook = buildDailyScanWorkbook(report);
      filename = `BlueRock-Scan-Daily-${report.reportDate}${report.current ? '-provisional' : ''}.xlsx`;
    }
