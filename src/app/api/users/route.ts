...
export async function GET(request:NextRequest){
  try{
    ...
    const rows=await db()`
      select u.id,u.username,u.full_name,u.role,u.active,u.assigned_location_id,u.scanner_access,
...
export async function POST(request:NextRequest){
  try{
    ...
    const scannerAccess=p.data.role==='SCANNER'?(p.data.scannerAccess??'MATERIALS'):'MATERIALS';
    ...
    const rows=await sql`
      insert into ims_users(username,password_hash,full_name,role,assigned_location_id,scanner_access)
      values(${p.data.username},${passwordHash},${p.data.fullName},${p.data.role},${assignedLocationId},${scannerAccess})
      returning id,username,full_name,role,active,assigned_location_id,scanner_access,created_at
    `;
