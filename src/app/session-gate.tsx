...
export default function SessionGate({ children }: { children: ReactNode }) {
  ...
  const check = useCallback(async () => {
    ...
    try {
      ...
      // A malformed saved profile must never crash the root login page.
      if (stored) { try { JSON.parse(stored); } catch { localStorage.removeItem('br_user'); } }
      if (!token) { setSession(null); setState('signedOut'); return; }
      ...
      if (reason instanceof ApiError && reason.status === 401) {
        setSession(null); setState('signedOut'); return;
      }
      setError(reason instanceof Error ? reason.message : 'Unable to verify your session.');
      // A network/database failure is NOT a bad password. Keep saved credentials
      // for Retry; never log out a working session because the database is busy.
      setState('error');
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (state === 'ready' && session && (pathname === '/login' || (pathname === '/' && session.user.role === 'SCANNER'))) {
      router.replace(session.user.role === 'SCANNER' ? '/scan' : '/?view=dashboard');
    }
  ...
  if (state !== 'ready' || !session || pathname === '/login' || (pathname === '/' && session.user.role === 'SCANNER') || (pathname === '/scanner-config' && session.user.role !== 'ADMIN')) {
    return <main className="dataLoading" role="status"><h2>Checking your session...</h2></main>;
  }
