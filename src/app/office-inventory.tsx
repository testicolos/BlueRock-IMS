...
export default function OfficeInventory({locations,api,notify}:{locations:Location[];api:Api;notify:(message:string)=>void}){
  ...
  return <>
    ...
    <section className="panel"><div className="panelHead"><div><h2>Validation history</h2><p>Select a request to review validated and pending assets.</p></div></div><label>Validation request<select value={selectedSessionId} onChange={e=>{setSelectedSessionId(e.target.value);void load(e.target.value)}}><option value="">Current / latest</option>{sessions.map(row=><option key={row.id} value={row.id}>{row.status==='OPEN'?'Active':'Closed'} · {formatDate(row.started_at)} · {row.validated}/{row.total}</option>)}</select></label>{targets.length>0&&<div className="tableScroll"><table><thead><tr><th>Barcode</th><th>Item</th><th>Owner</th><th>Location</th><th>Result</th><th>Validated by</th><th>Time</th></tr></thead><tbody>{targets.map(row=><tr key={row.id}><td><code>{row.barcode}</code></td><td>{row.name}</td><td>{row.owner_name||'Unassigned'}</td><td>{row.location_name||'Unassigned'}</td><td>{row.validated_at?'Validated':'Not validated'}</td><td>{row.validated_by_name||'—'}</td><td>{formatDate(row.validated_at||undefined)}</td></tr>)}</tbody></table></div>}</section>
