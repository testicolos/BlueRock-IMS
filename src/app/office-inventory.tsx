...
export default function OfficeInventory({locations,api,notify}:{locations:Location[];api:Api;notify:(message:string)=>void}){
  ...
  useEffect(()=>{void load()},[]);
  useEffect(()=>{const timer=window.setInterval(()=>void load(selectedSessionId||undefined),10_000);return()=>window.clearInterval(timer)},[selectedSessionId]);
  ...
  const shown=useMemo(()=>items.filter(item=>{
    ...
    const statusMatches=status==='ALL'||(active?item.validation_status===status:(status==='VALIDATED'?Boolean(item.last_validated_at):!item.last_validated_at));
    return statusMatches&&(!term||searchable.includes(term));
  }),[items,query,status,active]);
  ...
  return <>
    <section className="pageHero"><div><span className="eyebrow">OFFICE ASSET CONTROL</span><h2>Office Inventory</h2><p>Laptops, printers, displays and other office devices. Scan anytime; validation requests remain available for formal checks. Live scan status refreshes every 10 seconds.</p></div><div className="officeHeroActions"><button className="secondary" onClick={()=>{setSelected(null);setModal('add')}}><Plus size={17}/> Add office item</button><a className="primary" href="/office-scan"><Barcode size={17}/> Open office scanner</a>{active?<button className="secondary" disabled={busy} onClick={()=>void close()}><Square size={16}/> End validation</button>:<button className="secondary" disabled={busy} onClick={()=>void start()}><PlayCircle size={17}/> Start validation request</button>}</div></section>
