...
type ScannerAccess={scanAccess:'MATERIALS'|'OFFICE'|'BOTH';materials:boolean;office:boolean};
...
export default function ScanPage(){
  ...
  const[locations,setLocations]=useState<Location[]>([]);const[match,setMatch]=useState<Match|null>(null);const[attemptId,setAttemptId]=useState('');const[geo,setGeo]=useState<Stamp|null>(null);const[locationId,setLocationId]=useState('');const[notes,setNotes]=useState('');const[reportIssue,setReportIssue]=useState(false);const[issueType,setIssueType]=useState('Damaged equipment');const[unitPhoto,setUnitPhoto]=useState('');const[defectPhoto,setDefectPhoto]=useState('');const[access,setAccess]=useState<ScannerAccess|null>(null);const[busy,setBusy]=useState(false);const[scanning,setScanning]=useState(false);const[error,setError]=useState('');const[message,setMessage]=useState('');
  const token=()=>localStorage.getItem('br_token')||'';
  useEffect(()=>{const t=token();if(!t){window.location.href='/';return}let cancelled=false;(async()=>{try{const accessRes=await fetch('/api/scanner-access',{headers:{Authorization:`Bearer ${t}`},cache:'no-store'});const accessBody=await accessRes.json();if(!accessBody.success)throw new Error(accessBody.error?.message||'Unable to load scanner access');if(cancelled)return;setAccess(accessBody.data);if(accessBody.data.materials){const dataRes=await fetch('/api/app-data',{headers:{Authorization:`Bearer ${t}`},cache:'no-store'});const dataBody=await dataRes.json();if(!cancelled&&dataBody.success)setLocations(dataBody.data.locations||[])}}catch(reason){if(!cancelled)setError(reason instanceof Error?reason.message:'Unable to load scanner access')}})();return()=>{cancelled=true;stopCamera()}},[]);
  ...
  async function submit(){if(!match||!attemptId||!geo)return;if(reportIssue&&!defectPhoto){setError('Take or attach a defect photo before creating the defect report.');return}setBusy(true);setError('');setMessage('');try{const res=await fetch('/api/scans',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token()}`},body:JSON.stringify({barcode:match.barcode,locationId:locationId||undefined,notes:notes||undefined,reportIssue,issueType:reportIssue?issueType:undefined,evidenceImageUrl:unitPhoto||undefined,issueImageUrl:reportIssue?defectPhoto||undefined:undefined,validationAttemptId:attemptId,captureMethod:'CAMERA',clientTransactionId:crypto.randomUUID(),...geo})});const body=await res.json();if(!body.success)throw new Error(body.error?.message||'Unable to save scan');const base=body.data.transfer?`Scan saved. Transfer to ${body.data.transfer.destination.name} is awaiting approval.`:'Scan saved. Current location was left unchanged.';setMessage(body.data.issue?base+' Defect report created.':base);setMatch(null);setAttemptId('');setGeo(null);setLocationId('');setNotes('');setReportIssue(false);setIssueType('Damaged equipment');setUnitPhoto('');setDefectPhoto('')}catch(reason){setError(reason instanceof Error?reason.message:'Unable to save scan')}finally{setBusy(false)}}
  ...
  return <main style={shell}><div style={{maxWidth:900,margin:'0 auto'}}>
    ...
    <section style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:12,marginBottom:18}}>
      {access?.materials&&<div style={{...modeCard,borderColor:'#ff8a46',background:'#fff5ec'}}>
      ...
      {access?.office&&<a href="/office-scan" style={{...modeCard,borderColor:'#ff8a46',background:'#fff0e3'}}>
    ...
    {!access&&<section style={card}><strong>Loading scanner access...</strong></section>}
    {access?.materials&&<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:16}}>
      <section style={card}>
      ...
      <label style={{display:'flex',alignItems:'center',gap:9,marginTop:16,padding:12,border:'1px solid #e9a17a',borderRadius:10,background:'#fff1e8',fontWeight:800}}>
        <input type="checkbox" checked={reportIssue} onChange={e=>{const checked=e.target.checked;setReportIssue(checked);if(!checked)setDefectPhoto('')}}/>
    ...
    {access&&!access.materials&&access.office&&<section style={{...card,marginTop:4}}><strong>Office Inventory access only</strong><p style={{marginBottom:0,color:'#666'}}>This scanner account is configured for Office Inventory scanning. Use the Office Inventory option above.</p></section>}
  </div></main>;
}
