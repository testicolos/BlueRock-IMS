...
export class ApiError extends Error {
  constructor(message: string, public status: number, public requestId: string, public code: string) {
    super(requestId && !message.includes(requestId) ? `${message} Reference: ${requestId}` : message);
    this.name = 'ApiError';
  }
}
...
// Browser diagnostics are a bounded list of metadata, never form data or tokens.
export function getClientDiagnostics(): ClientDiagnostic[] {
  try {
    ...
    ).map(row => ({ time: row.time, operation: row.operation, method: row.method, status: row.status,
      durationMs: row.durationMs, requestId: row.requestId, code: row.code }));
  } catch { return []; }
}

function record(row: ClientDiagnostic) {
  try { sessionStorage.setItem(DIAGNOSTICS_KEY, JSON.stringify([...getClientDiagnostics(), row].slice(-100))); } catch { /* Optional diagnostics must never block login. */ }
...
export async function apiRequest<T>(path: string, options: RequestInit = {}, timeoutMs = 20_000): Promise<T> {
  ...
  try {
    ...
    if (controller.signal.aborted) {
      ...
      throw new ApiError('The request took too long. Check your connection and try again. A save may already have completed; refresh before resubmitting it.', 0, reference, code);
    }
    code = 'NETWORK_ERROR';
    ...
    record({ time: new Date().toISOString(), operation: operationFor(path), method: options.method || 'GET', status,
      durationMs: Math.round(performance.now() - started), requestId: reference, code });
  }
}
